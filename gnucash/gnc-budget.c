...
//*********************************************************************************
GncBudget*
gnc_book_lookup_budget_by_guid (const QofBook *book, const GncGUID *guid)
{
    return gnc_budget_lookup (guid, book);
}
...
static void match_name (QofInstance *ent, gpointer data)
{
    GncBudget *bgt = GNC_BUDGET(ent);
    struct bgt_name *bgt_name_str = (struct bgt_name*) data;

    if (g_strcmp0 (gnc_budget_get_name (bgt), bgt_name_str->name) == 0)
    {
        bgt_name_str->bgt = bgt;
    }
}
...
{
    QofCollection *col;
    struct bgt_name bgt_name_str;

    g_return_val_if_fail(book, NULL);
    g_return_val_if_fail(name, NULL);

    bgt_name_str.name = name;

    col = qof_book_get_collection (book, GNC_ID_BUDGET);
    qof_collection_foreach (col, match_name, &bgt_name_str);

    if (bgt_name_str.bgt != NULL)
        return bgt_name_str.bgt;

    return NULL;
}

GncBudget*
gnc_book_get_default_budget (QofBook *book)
{
    return gnc_budget_get_default (book);
}

const GncGUID*
gnc_book_get_default_budget_guid (QofBook *book)
{
    g_return_val_if_fail(book, NULL);

    return qof_book_get_default_budget_guid (book);
}

void
gnc_book_set_default_budget_guid (QofBook *book, const GncGUID* guid)
{
    ...
    qof_book_set_default_budget_guid (book, guid);
